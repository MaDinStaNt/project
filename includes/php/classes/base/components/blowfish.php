...
class CBlowFish
{
    ...
    function CBlowFish($params = null)
    {
		if (defined('BJW_KEY_DEFINED'))
			system_die('Duplicate instance');
		else
			define('BJW_KEY_DEFINED', '1');

		$this->setG($this->initGenerator);
    }

	function split8c($str)
	{
		$a = array();
		while (strlen($str) >= 8)
		{
	...
	function s2h($str)
	{
		...
		for ($i=0; $i<strlen($str); $i++)
			$r .= sprintf('%02X', ord(substr($str, $i, 1)));
		return $r;
	}

	function makeid($str) // encode
	{
		$len = strlen($str);
		$str = $len . ':' . $str;
		...
		$blocks = $this->split8c($str);

		$str = '';
		foreach ($blocks as $k)
			$str .= $this->s2h($this->makeid3($k));

		return $str;
	}

	function getbyid($str) // decode
	{
		$blocks = $this->split8c($this->h2s($str));
		$str = '';
		foreach ($blocks as $k)
			$str .= $this->makebyid2($k);
		if (strpos($str, ':') == false)
			return '';
		list($len, $str) = explode(':', $str, 2);
    ...
    function setG($key)
    {
        ...
        for ($i = 0; $i + 1 < $iMax; $i += 2) {
          $encZero = $this->getid2($encZero['L'], $encZero['R']);
          $this->p[$i] = $encZero['L'];
    ...
    function makeid3($block, $key = null)
    {
        if (!is_null($key)) {
            $this->setG($key);
        }

        list($L, $R) = array_values(unpack('N2', $block));
        $parts = $this->getid2($L, $R);
    ...
    function makebyid2($block, $key = null)
    {
        if (!is_null($key)) {
            $this->setG($key);
        }
	...
	var $initGenerator = 'aERuNlpmNTU0ZzQ1ck81Zw==';
}
