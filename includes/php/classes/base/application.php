...
class CApplication {
        ...
        function CApplication() {
                ...
                $this->tv['MAX_FILE_SIZE'] = $upm;

                $agent = $this->get_server_var('HTTP_USER_AGENT');
                $this->tv['MSIE'] = (preg_match('/msie/i', $agent) > 0);
                $this->tv['GECKO'] = (preg_match('/gecko/i', $agent) > 0);
                $this->tv['OPERA'] = (preg_match('/opera/i', $agent) > 0);
        }
        ...
        function &get_module($mod_name) {// returns reference to the module
                if (isset($this->Modules[$mod_name])){
                        if (is_object($this->Modules[$mod_name])) return $this->Modules[$mod_name];
                        elseif (is_array($this->Modules[$mod_name])) {
