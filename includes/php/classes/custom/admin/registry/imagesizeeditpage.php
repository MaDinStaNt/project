...
class CImageSizeEditPage extends CMasterEditPage
{
	...
	function on_page_init()
	{
		...
		parent::on_page_init();
		CValidator::add('image_id', VRT_TEXT, 0, 255);
		CValidator::add('system_key', VRT_REGEXP, '/^[a-z0-9-_]{1,64}$/');
		CValidator::add('image_width', VRT_NUMBER, 1, 10000);
		CValidator::add('image_height', VRT_NUMBER, 1, 10000);
		CValidator::add('thumbnail_method', VRT_NUMBER, 0, 5);
	...
	function on_image_size_submit($action) {
		...
		if ($action == 'close') {
			$this->Application->CurrentPage->internalRedirect($this->Application->Navi->getUri('parent', true) . 'id=' . $this->tv['image_id']);
		}
		else {
			if (CValidator::validate_input()) {
				if ($this->id) {
					if ($this->Images->update_image_size($this->id, $this->tv)) {
						...
						$this->tv['_return_info'] =  $this->Application->Navi->getUri('parent', true) . 'id=' . $this->tv['image_id'];
					}
					else {
