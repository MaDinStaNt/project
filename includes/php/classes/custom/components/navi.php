...
class CNaviModule
{
        ...
        function CNaviModule(&$app)        {
                ...
                if (MOD_REWRITE) {
                        ...
                        $request = substr($request, 0, strcspn($request, '?'));                        // cutting off get parameters
                        if ($indexFileNamePos = strpos($request, 'index.php')) {                // cutting off goddamn index.php
                                $request = substr($request, 0, $indexFileNamePos);
                        }
                        $_SERVER['PHP_SELF'] = $request;                                                                // nasty hack... sorry, guys
