...
<td colspan="2">
	<%if id%>
	<h2><%IT:Localizer string="product_image" /%>: <%=title%></h2>
	<%else%>
	<h2><%IT:Localizer string="new_product_image" /%></h2>
	<%/if%>
</td>
</table>
<table cellpadding="3" cellspacing="0" class="form">
	<tr>
		<td width="100" align="right" nowrap><strong><label for="product_id"><%IT:Localizer string="product" /%>:</label></strong></td>
		<td><%it:input type="select" name="product_id" id="product_id" readonly="true" class="inp" /%></td>
	</tr>
	<tr>
		<td width="100" align="right" nowrap><strong><label for="type"><%IT:Localizer string="type" /%>:</label></strong></td>
		<td><%it:input type="select" name="type" id="type" class="inp" /%></td>
	</tr>
	<tr>
	...
	<%if image_filename%>
		<tr>
			...
			<td><img src="<%=HTTP%>pub/production/product/<%=product_id%>/<%=size%>/<%=image_filename%>" /></td>
		</tr>
	<%/if%>
	<tr>
		<%if image_filename%>
